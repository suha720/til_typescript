...
```ts
const 객체명: {property: 종류, 메소드: () => 리턴타입종류} = {
    property: value,
    메소드: function(){데이터 가공}
}
...
```ts
const presonInfo: { nickName: string; age: number; job: string } = {
  nickName: "츄츄",
  age: 23,
  job: "인형",
};
...
for (let key in iu) {
  console.log(key);
  console.log(iu[key]); // 오류 발생, 데이터 타입 블라블라
  console.log(typeof key);
}
```

- 1. 해결 방법 중 `keyof` 를 추천합니다.
...
for (let key in iu) {
  ...
  console.log(iu[key]); // 오류 발생, key 가 "key" 로 안들어가서? 발생
...
type member_1 = { 이름: string; 생일: string };

const 블랙핑크_멤버_제니: member_1 = {
  이름: "제니",
  생일: "96-01",
};
const 블랙핑크_멤버_로제: member_1 = {
  이름: "로제",
  생일: "97-02",
};
const 블랙핑크_멤버_지수: member_1 = {
  이름: "지수",
  생일: "95-01",
};
const 블랙핑크_멤버_리사: member_1 = {
  이름: "리사",
  생일: "97-03",
};
...
// new 로 만든 변수는 인스턴스라고 합니다.
const 학생_1 = new Student("1234", "이리");
```
...
function Student(_번호, _이름) {
  ...
  this.name = _이름;
  this.say = function () {
    console.log(`${this.name}의 학번은 ${this.no} 입니다.`);
  };
  this.hi = () => {
...
const 아이유: IUType = { job: "가수", age: 10, song: "마시메로" };

const { job, age, song }: IUType = 아이유;
const 아이돌: IUType = { job, age, song };
```
...
type IUType = {
  job: string;
  age: number;
  // `?` 옵션 : 속성이 있을수도 있고 없을수도 있다
  song?: string;
};

const 아이유: IUType = { job: "가수", age: 10, song: "마시메로" };

function 분해함수({ job, age }: IUType) {
  console.log(job);
  console.log(age);
  return { job, age };
}
